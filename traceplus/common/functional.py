import operator
from six import PY3, PY2, with_metaclass
...
#####################################################################################################
# the section below here is mostly copied from
# `https://github.com/ionelmc/python-lazy-object-proxy/blob/master/src/lazy_object_proxy/utils.py`
# with BSD License
#####################################################################################################
...
class LazyObjectProxy(with_metaclass(_ProxyMetaType)):
    """
    ...
    def __wrapped__(self, __getattr__=object.__getattribute__, __setattr__=object.__setattr__,
        ...
        try:
            ...
            try:
                ...
                raise ValueError("LazyProxy hasn't been initiated: __factory__ is missing.")
            target = factory()
